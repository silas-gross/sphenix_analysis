...
LargeRLENC::LargeRLENC(const int n_run/*=0*/, const int n_segment/*=0*/, const float jet_min_pT/*=0*/, const bool data/*=false*/, const bool cluster/*=false*/, const bool pedestal, std::fstream* ofs/*=nullptr*/, const std::string vari/*="E"*/, const std::string& name/* = "LargeRLENC"*/) 
{
	...
	std::string truth_thresh_s="_"+std::to_string((int)truth_thresh)+"_MeV_threshold";
	
	fc=new MethodHistograms("Full_CAL"+allcal_thresh_s, 1.5*PI, 0.1); 
	fe=new MethodHistograms("Full_EMCAL"+emcal_thresh_s, 1.5*PI, 0.1);
	fi=new MethodHistograms("Full_IHCAL"+ihcal_thresh_s, 1.5*PI, 0.1);
	fo=new MethodHistograms("Full_OHCAL"+ohcal_thresh_s, 1.5*PI, 0.1); //Full OHCAL has R ~ 3.83 (delta eta ~ 2.2, delta phi ~pi)

	tc=new MethodHistograms("Towards_Region_CAL"+allcal_thresh_s, 1.5*PI, 0.1);
	te=new MethodHistograms("Towards_Region_EMCAL"+emcal_thresh_s, 1.5*PI, 0.1);
	ti=new MethodHistograms("Towards_Region_IHCAL"+ihcal_thresh_s, 1.5*PI, 0.1);
	to=new MethodHistograms("Towards_Region_OHCAL"+ohcal_thresh_s, 1.5*PI, 0.1); //Towards_Region OHCAL has R ~ 3.83 (delta eta ~ 2.2, delta phi ~2pi/3)

	ac=new MethodHistograms("Away_Region_CAL"+allcal_thresh_s, 1.5*PI, 0.1);
	ae=new MethodHistograms("Away_Region_EMCAL"+emcal_thresh_s, 1.5*PI, 0.1);
	ai=new MethodHistograms("Away_Region_IHCAL"+ihcal_thresh_s, 1.5*PI, 0.1);
	ao=new MethodHistograms("Away_Region_OHCAL"+ohcal_thresh_s, 1.5*PI, 0.1); //Away_Region OHCAL has R ~ 3.83 (delta eta ~ 2.2, delta phi ~2pi/3)

	trc=new MethodHistograms("Transverse_Region_CAL"+allcal_thresh_s, 1.5*PI, 0.1);
	...
	h_total_E_dc=new TH1F("h_total_E_dc", Form("E_{true} - E_{cut} in all calorimeters, E_{tow} #geq %d MeV; #Delta E[GeV]; N_{events}", (int)allcal_thresh), 600, -300.5, 299.5);
	h_ohcal_E_dc=new TH1F("h_ohcal_E_dc", Form("E_{true} - E_{cut} in ohcal, E_{tow} #geq %d MeV; #Delta E[GeV]; N_{events}", (int)ohcal_thresh), 600, -300.5, 299.5);
	h_ihcal_E_dc=new TH1F("h_ihcal_E_dc", Form("E_{true} - E_{cut} in ihcal, E_{tow} #geq %d MeV; #Delta E[GeV]; N_{events}", (int)ihcal_thresh), 600, -300.5, 299.5);
	h_emcal_E_dc=new TH1F("h_emcal_E_dc", Form("E_{true} - E_{cut} in emcal, E_{tow} #geq %d MeV; #Delta E[GeV]; N_{events}", (int)emcal_thresh), 600, -300.5, 299.5);
	h_truth_E_dc=new TH1F("h_truth_E_dc", "E_{true} - E_{cut} Truth Particles, E_{particle} #geq 1 GeV; #Delta E[GeV]; N_{events}", 600, -300.5, 299.5);
	...
	h_whole_truth_eta_E=new TH1F("h_whole_truth_eta_E", "Truth energy deposition relative to leading jet; #Delta #eta; E [GeV]", 100, -1.1, 1.1); 	
	h_jet_truth_eta_E=new TH1F("h_jet_truth_eta_E", "Truth jet energy deposition relative to leading jet; #Delta #eta; E [GeV]", 100, -1.1, 1.1); 	
	h_UE_truth_eta_E=new TH1F("h_UE_truth_eta_E", "Truth Underlyding eveent energy deposition relative to leading jet; #Delta #eta; E [GeV]", 100, -1.1, 1.1); 	
	h_whole_truth_eta=new TH1F("h_whole_truth_eta", "Truth particle deposition relative to leading jet; #Delta #eta; N", 100, -1.1, 1.1); 	
	h_jet_truth_eta=new TH1F("h_jet_truth_eta", "Truth jet particle deposition relative to leading jet; #Delta #eta; N", 100, -1.1, 1.1); 	
	h_UE_truth_eta=new TH1F("h_UE_truth_eta", "Truth Underlyding event particle deposition relative to leading jet; #Delta #eta; N", 100, -1.1, 1.1); 	
	h_whole_truth_phi_E=new TH1F("h_whole_truth_phi_E", "Truth energy deposition relative to leading jet; |#Delta #varphi|; E [GeV]", 100, -1.1, 4.0); 	
	h_jet_truth_phi_E=new TH1F("h_jet_truth_phi_E", "Truth jet energy deposition relative to leading jet; |#Delta #varphi|; E [GeV]", 100, -0.5, 4.0); 	
	h_UE_truth_phi_E=new TH1F("h_UE_truth_phi_E", "Truth Underlyding eveent energy deposition relative to leading jet; |#Delta #varphi|; E [GeV]", 100, -0.5, 4.0); 	
	h_whole_truth_phi=new TH1F("h_whole_truth_phi", "Truth particle deposition relative to leading jet; |#Delta #varphi|; N", 100, -0.5, 4.0); 	
	h_jet_truth_phi=new TH1F("h_jet_truth_phi", "Truth jet particle deposition relative to leading jet; |#Delta #varphi|; N", 100, -0.5, 4.0); 	
	...
	try{
		...
		//	if(jets->size() == 0 ) jets=getJets("Truth", radius, topNode);
			...
			/*	if(!jets || jets->size() == 0){
				...
				//try every possible style no sub for run
			}
			if(jets) std::cout<<"Jet algo: " <<jets->get_algo()<<std::endl;
	/*		else {
				foundJetConts=true;
				}*/
		}
...
/*		PHG4TruthInfoContainer *truthinfo=findNode::getClass<PHG4TruthInfoContainer>(topNode, "G4TruthInfo");
	...
	if(!isDijet || !triggered_event){ //stores some data about the bad cuts to look for any arrising structure
		...
		if(isRealData )eventCut->dumpStatus();
/*		if(jets->size() > 0){
			int l=0;
			for(auto j:*jets){
				l++;
	...
	*/
		...
		h_truth_E_c->Fill(e5);
		float dc_o=ohcal_energy-e3; 
		float dc_i=ihcal_energy-e2; 
		float dc_e=emcal_energy-e1; 
		float dc_t=total_energy-e4;
		h_total_E_dc->Fill(dc_t);	
		h_ohcal_E_dc->Fill(dc_o);	
		...
		for(auto l:truth_pts){
			...
			h_whole_truth_phi->Fill(std::abs(dphi));
			h_whole_truth_eta->Fill(deta);
			h_whole_truth_phi_E->Fill(std::abs(dphi), l.second);
		...
		for(auto j:*jets){
				...
				if(source < Jet::SRC::PARTICLE - 1 || source>  Jet::SRC::CHARGED_PARTICLE){
					...
					if(truth_particles.find(id) == truth_particles.end()){
						...
						if(abs(pid) <= 11 || abs(pid) > 18 ){
							...
							h_jet_truth_phi->Fill(std::abs(dphi_p));
							h_jet_truth_eta->Fill(deta_p);
							h_jet_truth_phi_E->Fill(std::abs(dphi_p), E);
							h_jet_truth_eta_E->Fill(deta_p, E);
						}
					}
				}
			...
			for(auto tp:truth_particles)
			{
				...
				h_UE_truth_phi->Fill(std::abs(dphi));
				h_UE_truth_eta->Fill(deta);
				h_UE_truth_phi_E->Fill(std::abs(dphi), E);
