...
{
	if [[ $nfiles -eq 0 ]]; then 
		nfiles=`wc -l < MC/${triggertype}_data/jet_density_${filedensity}.list`
	fi	
	for i in $(seq 0 ${nfiles}); do 
		...
		echo "${blanklines[2]}" $(pwd) ${options} ${outDir} $MYINSTALL >> $condor_file
...
{
	if [[ $nfiles -eq 0 ]]; then 
		nfiles=`wc -l < real_data/$run_data/jet_density_${filedensity}.list`
	fi	
	for i in $(seq 0 ${nfiles}); do 
		j=$(( i+1 ))
		if [ $i -eq $nfiles ]; then 
	...
	while [ $# -gt 0 ]; do 
		case $1 in 
			-h | --help)
				...
				echo " "
				echo " -d, --data	Which run number for real data"
				echo " "
				exit 0 
				;;
			-v | --verbose)
			...
			-d | --data) 
				dosim=false
				run_data=$(extract_argument $@)
				if [ "$verbose" = true ]; then
					echo "working on run ${run_data}"
				fi
				exit 1
				;;
			*)
