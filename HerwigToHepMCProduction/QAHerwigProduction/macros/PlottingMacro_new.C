...
)
{
	...
	for(int i = 1; i<(int)base->size(); i++)
	{
		if ( i > 0 ) 
		{
			...
			total_ratio->GetYaxis()->SetRangeUser(0, 1);
	}
	pads->at(1)->cd();
	data_leg->Draw();
	head_leg->Draw();
	...
	)
{
	...
	for(int i=0; i<3; i++)
	{
		TLegend* l_head	= new TLegend(0.6, 0.7, 0.8, 1); 
		TLegend* l_data	= new TLegend(0.6, 0.4, 0.8, 0.7);
		co->SetLegend(l_data);
...
void PlotCombinedSpectrum(std::vector<TFile*>* fs, std::vector<std::string> cutnames)
{
	...
	for(int i=0; i<2; i++)
	{
		...
		for(int j=0; j<2; j++)
		{
			...
			if(j==1) LJ="Lead Jets";
			TLegend* l_head	= new TLegend(0.05, 0.8, 0.3, 1);
			TLegend* l_data = new TLegend(0.32, 0.7, 1, 0.95);
	...
	TCanvas* cA=new TCanvas("Combined_all", "Combined_all");
	TCanvas* cL=new TCanvas("Combined_lead", "Combined_lead");
	std::vector<TPad*>* pA = co->AddPads(cA);
	std::vector<TPad*>* pL = co->AddPads(cL);
	pA->at(1)->cd();
	pA->at(1)->SetLogy();
	ha1->Draw("pmc plc");
	pa1->Draw("pmc plc same");
	ha1->SetMarkerStyle(20);
	pa1->SetMarkerStyle(24);
	TH1F* ra1 = co->GetRatioPlot(ha1, pa1);

	TLegend* l1	= new TLegend(0.7, 0.8, 1,1);
	TLegend* l2	= new TLegend(0.7, 0.6, 1, 0.8);
	co->SetsPhenixHeaderLegend(l1, "none");
	co->SetLegend(l2);
	
	l1->AddEntry("","Combined Samples all jets","");
	l2->AddEntry(ha1, "Herwig", "pl");
	l2->AddEntry(pa1, "Pythia", "pl");
	l2->AddEntry(ra1, "Herwig / Pythia", "pl");
	l1->Draw();
	l2->Draw();
	pA->at(0)->cd();
	ra1->Draw("pmc plc");
	ra1->SetYTitle("Herwig / Pythia");
	ra1->GetYaxis()->SetRangeUser(0,2);
	ra1->SetMarkerStyle(39);
	cA->cd();
	pA->at(1)->Draw();
	pA->at(0)->Draw();
	...
	pL->at(1)->SetLogy();
	hj1->Draw("pmc plc");
	pj1->Draw("pmc plc same");
	hj1->SetMarkerStyle(20);
	pj1->SetMarkerStyle(24);
	TH1F* rj1 = co->GetRatioPlot(hj1, pj1);

	TLegend* l3	= new TLegend(0.7, 0.8, 1,1);
	TLegend* l4	= new TLegend(0.7, 0.6, 1, 0.8);
	co->SetsPhenixHeaderLegend(l3, "none");
	co->SetLegend(l4);
	
	l3->AddEntry("","Combined Samples Lead jets","");
	l4->AddEntry(hj1, "Herwig", "pl");
	l4->AddEntry(pj1, "Pythia", "pl");
	l4->AddEntry(rj1, "Herwig / Pythia", "pl");
	l3->Draw();
	l4->Draw();
	pL->at(0)->cd();
	rj1->Draw("pmc plc");
	rj1->GetYaxis()->SetRangeUser(0,2);
	rj1->SetYTitle("Herwig / Pythia");
	rj1->SetMarkerStyle(39);
	cL->cd();
	pL->at(1)->Draw();
	pL->at(0)->Draw();
	...
	PlotBothThePlots(
		...
		&All, &Lead,
		co, cutnames);
	std::vector<TCanvas*> Canvases_to_Print;
	for(int i = 0; i<(int) All.size();  i++) Canvases_to_Print.push_back(All.at(i));
	for(int i = 0; i<(int) Lead.size(); i++) Canvases_to_Print.push_back(Lead.at(i));
	for(int i = 0; i< 2; i++)
	       	for(int j = 0; j < 2; j++)
			Canvases_to_Print.push_back(RatioCanvi.at(i)->at(j));
	Canvases_to_Print.push_back(cA);
	Canvases_to_Print.push_back(cL);

	co->DumpHistos("~/Herwig_comb_pt_QA.pdf", &Canvases_to_Print);
	for(int i=0; i<(int)Canvases_to_Print.size(); i++)
	       	Canvases_to_Print.at(i)->SaveSource(std::format("{}.C", Canvases_to_Print.at(i)->GetName()).c_str());
...
void PlotEventObs(TH1F* herwig_obs, TH1F* pythia_obs, TCanvas* c1)
{
	...
	s->GetYaxis()->SetRangeUser(herwig_obs->GetMinimum()*0.33, 3*(herwig_obs->GetMaximum()));
...
void PlotEventObs(TH1F* herwig_particle, TH1F* pythia_particle, std::map<std::string, particle_colls*>* h_pc, std::map<std::string, particle_colls*>* p_pc, std::map<std::string, particle_colls*>* r_pc, TCanvas* c1)
{
	...
	herwig->GetYaxis()->SetRangeUser(herwig_particle->GetMinimum()*0.33, herwig_particle->GetMaximum()*3);
	l_data->Draw();
	l_header->Draw();
	pads->at(0)->cd();
...
void PlotJetObs(std::vector<TH1F*>* herwig_obs, int n_herwig, std::vector<TH1F*>* pythia_obs, int n_pythia, TCanvas* obs_canv)
{
	...
	herwig->GetYaxis()->SetRangeUser(herwig_obs->at(0)->GetMinimum()*0.33, herwig_obs->at(0)->GetMaximum()*3);
	l_data->Draw();
	l_header->Draw();
	obs_canv->cd();
	pads->at(0)->Draw();
...
{
	//just does the plotting of one variable at a tie
	if(!herwig_obs || !pythia_obs) return;
	TLegend* l_data=new TLegend(0.6, 0.3, 1, 0.5);
	TLegend* l_header=new TLegend(0.8, 0.6, 1, 1);
	conf->SetsPhenixHeaderLegend(l_header, tag);
...
void DoAllThePlotting(TFile* herwig_file, TFile* pythia_file, std::string trigger_tag="Jet30", float herwig_xs=1., float pythia_xs=1.)
{
	...
	{
		PlotJetPlots(top_dirs["herwig photons"], top_dirs["pythia photons"], Canvi);
	}
